...
## 从HBase读取RDD
SparkContext的**newAPIHadoopRDD** API函数可以加载HBase表中的数据并生成RDDs。
### 使用newAPIHadoopRDD
```scala
object ReadData {
    def main(args: Array[String]): Unit = {
        if(args.length < 2){
            System.err.println("Usage: ReadData <input table>")
            System.exit(1)
        }
        //创建HBase配置
        val hconf = HBaseConfiguration.create()
        hconf.set("hbase.zookeeper.quorum","hadoop01,hadoop02,hadoop04")
        //设置表名
        hconf.set(TableInputFormat.INPUT_TABLE,args(0))

        val sconf = new SparkConf().setAppName("ReadRDDFromHBase")
        val sc = new SparkContext(sconf);
        //加载HBase表数据生成RDD
